...
predicted_parts_path_list = [ "./predicted_parts/track3" ]
...
for predicted_parts_path in predicted_parts_path_list:
...
assert len(predicted_docs) == len(original_docs) * len(predicted_parts_path_list)

new_docs = []
for orig_doc in original_docs:
    orig_id = orig_doc["id"]
    pred_docs = [d for d in predicted_docs if d["id"] == orig_id]

    # print([list(l) for pred_doc in pred_docs for l in zip(*pred_doc["pred_ner"])])
    pred_start_chars, pred_end_chars, pred_types = [], [], []
    for pred_doc in pred_docs:
        pred_start_chars_onedoc = []
        pred_end_chars_onedoc = []
        pred_types_onedoc = []
        curr_preds = [list(l) for l in zip(*pred_doc["pred_ner"])]
        if len(curr_preds) > 0:
            pred_start_chars_onedoc, pred_end_chars_onedoc, pred_types_onedoc, _, _ = curr_preds
        pred_start_chars.extend(pred_start_chars_onedoc)
        pred_end_chars.extend(pred_end_chars_onedoc)
        pred_types.extend(pred_types_onedoc)
    ...
    preds = sorted(list(set(preds)), key = lambda x : x[0])
    origs = sorted(list(zip(orig_start_chars, orig_end_chars, orig_types)), key = lambda x : x[0])
